...
| AI Gateway 不保存业务正文 | 不保存完整 prompt、embedding payload、rerank 文档正文、provider 原始响应体或用户上传文档全文。 |
...
| topN 规则 | 请求 `top_n` 优先；缺省时使用 profile `topN`；最终 effective topN 必须 `> 0`，并用于裁剪最终响应 `data`。 |
| 兼容响应 | 支持 provider 返回 `data[]` 或 `results[]`；`score` 或 `relevance_score` 会归一化为 `score`。 |
| 响应校验 | `index` 必须落在请求 documents 范围内；不允许重复；如 provider 返回 `document_id`，必须匹配对应请求文档 ID；最终响应必须有 `document_id`；provider 返回结果多于 effective topN 时，最终响应 `data` 必须截断到 effective topN。 |
...
当前未实现真实配额扣减、成本报表、Prometheus metrics 或 OpenTelemetry tracing。新增这些能力时，指标 label 只能使用低基数字段，不得包含 prompt、query、document text、embedding payload、API key 或 object key。
...
普通 CI 和本地默认回归使用受控 fake provider，不依赖外部模型服务：
...
| 样本 | 覆盖点 |
| --- | --- |
| `TestChatSmoke_*` / `TestChatStreamSmoke_*` | chat 非流式、stream、request id、API key 脱敏、provider 401/429/5xx/timeout 和 early close。 |
| `TestEmbeddingSmoke_ControlledProviderOpenAIShapeRecordsSummary` | embedding 成功路径，fake provider 返回 OpenAI-compatible `object=list`，验证 `/v1/embeddings`、Bearer API key、profile dimensions fallback、usage、调用摘要和不保存输入文本/向量/API key。 |
| `TestRerankSmoke_ControlledProviderResultsShapeRecordsSummary` | rerank 成功路径，fake provider 返回 `results[] + relevance_score + meta.tokens`，验证 `/v1/rerank`、`return_documents=false`、`top_n`、metadata、usage 映射、topN 截断、document text 不泄露。 |

真实 provider smoke 只在显式环境变量齐备时运行；没有开关时普通 `go test ./...` 会跳过：
...
可以只设置其中一种 operation model 环境变量来跑单项真实 provider smoke；`AI_GATEWAY_REAL_PROVIDER_SMOKE`、`AI_GATEWAY_REAL_PROVIDER_BASE_URL` 和 `AI_GATEWAY_REAL_PROVIDER_API_KEY` 仍必须同时存在。

## 当前测试覆盖和缺口

| 范围 | 当前覆盖 | 缺口 |
| --- | --- | --- |
| Chat | fake provider HTTP tests 覆盖非流式、流式、function-calling 字段、失败记录、request id、API key 脱敏和 env-gated 真实 provider 入口。 | 实际真实 provider 运行记录、stream cancel、provider 特异 chunk 行为。 |
| Embeddings | HTTP/service tests 覆盖 OpenAI shape、受控 fake provider 成功路径、profile model mismatch、count/index 校验、usage、调用摘要和脱敏；真实 provider smoke 入口默认跳过。 | 实际真实 provider 运行记录、不同维度/encoding_format 的真实兼容性验证。 |
| Rerankings | HTTP/service tests 覆盖 document_id 映射、document text 不泄露、provider response 校验、`data[]`/`results[]` 兼容归一、topN 截断和 env-gated 真实 provider 入口。 | 实际真实 provider 运行记录、更多 provider-specific rerank response shape。 |
