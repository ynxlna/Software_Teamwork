...
Owner service 只能保存 file 服务返回的内部 file reference，并在自己的资源边界内暴露业务资源 ID。例如：
...
Owner service 保存的内部 file reference 应只用于服务间调用，不作为公开 API 字段返回。
...
- Owner service 删除业务资源后，应协调 file reference 进入删除或延迟清理流程。
